...
<input placeholder="请输入验证码" type="text" id="${name}" name="${name}" maxlength="5" class="txt required" style="${inputCssStyle}"/>
...
<a href="javascript:" onclick="$('.${name}').attr('src','${pageContext.request.contextPath}/servlet/validateCodeServlet?'+new Date().getTime());" class="mid ${name}Refresh" style="${buttonCssStyle}">换一换</a>